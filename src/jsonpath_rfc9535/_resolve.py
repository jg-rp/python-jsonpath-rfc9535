...
def resolve(
    ...
    nodes: Iterable[Node] = [(data, (root,), data, None)]
    for segment in segments:
        nodes = resolve_segment(env, segment, nodes)
    return nodes


def resolve_segment(
    env: JSONPathEnvironment, segment: Segment, nodes: Iterable[Node]
) -> Iterable[Node]:
    match segment:
        case (_ast.CHILD_SEGMENT, _, selectors):
            for node in nodes:
                for selector in selectors:
                    yield from resolve_selector(env, selector, node)

        case (_ast.DESCENDANT_SEGMENT, _, selectors):
            for node in nodes:
                for node_ in visit(node, max_depth=env.max_recursion_depth):
                    for selector in selectors:
                        yield from resolve_selector(env, selector, node_)


def resolve_selector(
    env: JSONPathEnvironment, selector: Selector, node: Node
) -> Iterable[Node]:
    obj = node[0]
    ...
    match selector:
        ...
        case (_ast.FILTER_SELECTOR, _, expr):
            if type(obj) is dict:
                for k, v in obj.items():
                    if truthy(evaluate_expression(env, expr, k, v, node[2])):
                        yield new_child(node, k, v)

            elif type(obj) is list:
                for i, elem in enumerate(obj):
                    if truthy(evaluate_expression(env, expr, i, elem, node[2])):
                        yield new_child(node, i, elem)
...
def evaluate_expression(
    ...
    match expr:
        ...
        case (_ast.NOT_EXPR, _, right):
            return not truthy(
                evaluate_expression(env, right, current_key, current_value, root)
            )

        case (_ast.AND_EXPR, _, left, right):
            return truthy(
                evaluate_expression(env, left, current_key, current_value, root)
            ) and truthy(
                evaluate_expression(env, right, current_key, current_value, root)
            )
        ...
        case (_ast.EQ_EXPR, _, left, right):
            left_ = evaluate_expression(env, left, current_key, current_value, root)
            if isinstance(left_, NodeList) and len(left_) == 1:
                left_ = left_[0][0]

            right_ = evaluate_expression(env, right, current_key, current_value, root)
        ...
        case (_ast.ABSOLUTE_QUERY_EXPR, _, segments):
            return NodeList(resolve(env, segments, "$", root))

        case (_ast.RELATIVE_QUERY_EXPR, _, segments):
            return NodeList(resolve(env, segments, "@", current_value))
...
def visit(node: Node, *, max_depth: int) -> Iterable[Node]:
...
def new_child(node: Node, key: str | int, value: object) -> Node:
    return (value, node[1] + (key,), node[2], node)
